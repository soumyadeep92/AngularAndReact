...
const Header=()=>{
    ...
    <header className='fixed z-50 w-screen p-3 px-4 md:p-6 md:px-16'>
        <div className='hidden md:flex w-full h-full items-center justify-between'>
            ...
            <div className='flex items-center gap-8'>
                ...
                <div className='relative'>
                    <motion.img whileTap={{scale:0.6}} src={user?user.photoURL:Avatar} className='w-10 min-w-[40px] h-10 min-h-[40px] shadow-2xl rounded-full' alt='userprofile' onClick={login}/>
                    {
                        isMenu && (
                        ...
                        {
                            user && user.email==="soumyatechno2011@gmail.com" && (
        ...
        {/* <div className='flex items-center justify-hidden md:hidden w-full h-full'> */}
            {/* <Link to={'/'} className='flex items-center gap-2'>
                <img src={Logo} className='w-10 object-cover' alt='logo'/>
                <p className='text-headingColor text-xl font-bold'>City</p>
            </Link> */}
            {/* <div className='relative'>
                <motion.img whileTap={{scale:0.6}} src={user?user.photoURL:Avatar} className='w-10 min-w-[40px] h-10 min-h-[40px] shadow-2xl rounded-full' alt='userprofile' onClick={login}/>
                {
                    ...
                    {
                        user && user.email==="soumyatechno2011@gmail.com" && (
                            <Link to={'/createItem'}><p className='px-4 py-2 flex items-center cursor-pointer hover:bg-slate-100 transition-all duration-100 ease-in-out text-textColor text-base'>New Item <MdAdd/></p></Link>
                        )
                    }
                    <ul className='flex flex-col px-4 py-2 ml-auto'>
